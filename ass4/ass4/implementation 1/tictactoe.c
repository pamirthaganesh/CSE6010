 ...
 *      None
 */
void puzzle_solve(char **board, int grid_size, int **count_X_col, int **count_O_col, int  **count_Q_col, int **count_X_row,
                  int **count_O_row, int **count_Q_row){

    // Everytime we call the function, we pass the actual board, rather than a pointer to the board
    // This means that every time we call the function, a copy of the board is made
    // Therefore, when we return from the recursive call, we should go back to what the board was before the function call
    // However, for other purposes, we still need a pointer to the current board
    char **grid;
    grid = board;
...
//        printGrid(&grid, grid_size); // for debugging- take out when done
//        printf("\n");
...
//        printf("\n");
    ...
    if (checkEmpty(&grid, grid_size) == 0){
        ...
        if (rowDistinctionCheck(&grid, grid_size) == 0) {
            if (colDistinctionCheck(&grid, grid_size) == 0) {
                printf("Grid is unique");
                exit(0);
            }
        }
    }
    ...
    // If we do not meet the termination condition, we need to continue solving the board
        ...
        // If the grid is filled, then location_found will be 0
        // Then we can ignore the rest of the function and check the termination condition
        for (int i = 0; i < grid_size; i++) {
            for (int j = 0; j < grid_size; j++) {
...
//                printf("\n");
                ...
                puzzle_solve(board, grid_size, count_X_col, count_O_col, count_Q_col, count_X_row, count_O_row,
                             count_Q_row);
...
int rowDistinctionCheck(char ***grid, int grid_size){
    ...
    for (int i = 0; i < (grid_size); i++){
        for (int k = 0; k < (grid_size) ; k++){
            count = 0; // keep track of how many elements in the row are the same
            if(i!=k){ // condition required so that we don't compare a row with itself
                for (int j = 0; j < (grid_size ) ; j++){
                    if((*grid)[i][j] == (*grid)[k][j]){ // if two elements are the same, we add to the count
                        count = count+1;
                    }
                }
                if (count == grid_size) { // every single element of two rows are the same
                    return 1;
                }
            }
        }
...
int colDistinctionCheck(char ***grid,  int grid_size){
    ...
    for (int j = 0; j < (grid_size); j++){
        for (int k = 0; k < (grid_size) ; k++){
            count = 0; // keep track of how many elements in the column are the same
            if(j!=k){ // condition required so that we don't compare a column with itself
                for (int i = 0; i < (grid_size ) ; i++){
                    if((*grid)[i][j] == (*grid)[i][k]){ // if two elements are the same, we add to the count
                        count = count+1;
                    }
                }
            }

            if (count == grid_size){ // if all elements of two columns are the same, return 1
                return 1;
            }
