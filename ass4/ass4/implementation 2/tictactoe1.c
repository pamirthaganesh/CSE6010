...
#include "myboolean1.h"

// Define global variables
int flag_for_2_similar_row_char;
int flag_for_2_similar_col_char;
int flag_for_blank_between_2_similar_char_row;
int flag_for_blank_between_2_similar_char_col;
int flag_for_equal_no_of_character;

/*
 * Function: check_for_empty_cells
 * --------------------
 * Description:
...
/*
 * Function: check_for_2_similar_char_in_row
 * --------------------
 * Description:
...
void check_for_2_similar_char_in_row(char ***grid, int grid_size, int i, int j){
    ...
    if((*grid)[i][j] == (*grid)[i][j+1] && (*grid)[i][j] == 'X'){
        if((j>=1) && ((*grid)[i][j-1] == '?')){
            (*grid)[i][j-1] = 'O';
            flag_for_2_similar_row_char = 1; // set flag if change has been made
        }
...
void twoConsecutiveCol(char ***grid, int grid_size, int i, int j){
    ...
    if((*grid)[i][j] == (*grid)[i+1][j] && (*grid)[i][j] == 'X'){
        if((i>=1) && ((*grid)[i-1][j] == '?')){
            (*grid)[i-1][j] = 'O';
            flag_for_2_similar_col_char = 1; // set flag if change has been made
        }
...
/*
 * Function: check_for_blank_between_2_similar_char_row
 * --------------------
 * Description:
 *      Loop through the entire board
...
void check_for_blank_between_2_similar_char_row(char ***grid, int grid_size, int i, int j){
    ...
    if ((j==0) || (j==grid_size-1)){
        ...
        if (((*grid)[i][j-1] == 'O') && ((*grid)[i][j+1] == 'O') && ((*grid)[i][j] == '?')){ // if both cells are either side are O, then this cell must be an X
            (*grid)[i][j] = 'X';
            flag_for_blank_between_2_similar_char_row = 1; // set flag if change has been made
        }
...
/*
 * Function: check_for_blank_between_2_similar_char_col
 * --------------------
 * Description:
 *      Loop through the entire board
...
void check_for_blank_between_2_similar_char_col(char ***grid, int grid_size, int i, int j){
    ...
    if ((i==0) || (i==grid_size-1)){
        ...
        if (((*grid)[i-1][j] == 'O') && ((*grid)[i+1][j] == 'O') && ((*grid)[i][j] == '?')){ // if both cells are either side are O, then this cell must be an X
            (*grid)[i][j] = 'X';
            flag_for_blank_between_2_similar_char_col = 1; // set flag if change has been made
        }
...
/*
 * Function: check_for_equal_no_of_character
 * --------------------
 * Description:
 *      Loop through the entire board
    ...
    for(int j =0; j<grid_size; j++){
        if((*count_X_col)[j] == grid_size/2){
            for(int i = 0; i<grid_size; i++){
                if((*grid)[i][j] == '?'){
                    (*grid)[i][j] = 'O';
                    flag_for_equal_no_of_character = 1; // set the flag if change is made
                }
            }
        }
    }
...
void print_the_grid(char ***grid, int grid_size){
    void print(char *string,int grid_size)
  {
    int i;
 ...
 * --------------------
 * Description:
 *      Check to see if putting a certain letter at a specific character (row,column) location will result in 3 of that letter in a row
 ...
 *      None
 */
void solve_the_puzzle(char ***grid, int grid_size, int **count_X_col, int **count_O_col, int  **count_Q_col, int **count_X_row,
    ...
    flag_for_2_similar_row_char = 1;
    flag_for_2_similar_col_char = 1;
    flag_for_blank_between_2_similar_char_row = 1;
    flag_for_blank_between_2_similar_char_col = 1;
    flag_for_equal_no_of_character = 1;
    ...
    if ((check_for_empty_cells(grid, grid_size) == 0)
    && (check_for_row_duplicate(grid, grid_size)==0)
    && (check_for_col_duplicate(grid, grid_size)==0)){
        print_the_grid(grid, grid_size);
        exit(0);
    }
    ...
    // If we do not meet the termination condition, we need to continue solving the board
        ...
        while ((flag_for_2_similar_row_char == 1) || (flag_for_2_similar_col_char == 1) || (flag_for_blank_between_2_similar_char_row == 1)
        || (flag_for_blank_between_2_similar_char_col == 1) || (flag_for_equal_no_of_character == 1)){
             ...
             * Since there cannot be more than 2 consecutive of the same letter
             */
            flag_for_2_similar_row_char = 0;
            for (int i=0; i<grid_size; i++){
                for (int j=0; j<grid_size-1; j++){
                    check_for_2_similar_char_in_row(grid, grid_size, i, j);
                }
            }
            print_the_grid(grid, grid_size);
            printf("\n");
            ...
            for (int i=0; i<grid_size; i++){
                for (int j=0; j<grid_size; j++){
                    check_for_blank_between_2_similar_char_row(grid, grid_size, i, j);
                }
            }
            print_the_grid(grid, grid_size); // for debugging- take this out when done
            printf("\n");
            ...
            for (int i=0; i<grid_size; i++){
                for (int j=0; j<grid_size; j++){
                    check_for_blank_between_2_similar_char_col(grid, grid_size, i, j);
                }
            }
            print_the_grid(grid, grid_size);
            printf("\n");
            ...
            flag_for_equal_no_of_character = 0;
            check_for_equal_no_of_character(grid, grid_size, count_X_col, count_O_col, count_Q_col, count_X_row, count_O_row, count_Q_row);
            print_the_grid(grid, grid_size);
            printf("\n");
        }
        ...
        if (((*count_X_row)[new_i] < grid_size/2)
            ...
            && (check_for_possible_violation(grid, new_i, new_j, grid_size, 'X') == 0)){
            ...
            solve_the_puzzle(grid, grid_size, count_X_col, count_O_col, count_Q_col, count_X_row, count_O_row, count_Q_row);
                 ...
                 && (check_for_possible_violation(grid, new_i, new_j, grid_size, 'O') == 0)) {
...
/*
 * Function: check_for_row_duplicate
 * --------------------
 * Description:
...
/*
 * Function: check_for_col_duplicate
 * --------------------
 * Description:
